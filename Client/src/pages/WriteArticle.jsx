        ...
        <p className="mt-6 text-sm font-medium">Article Length</p>
         <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {articleLength.map((item,idx) => (
             <span onClick={()=>{setSelectedLength(item)}} key={idx} className={`text-xs px-4 py-1  border-2 rounded-full cursor-pointer ${selectedLength.text ===item.text ? 'bg-blue-50 text-blue-700' :'text-gray-500 border-gray-300'}`}>{item.text}</span>
          ))}
         </div>

         <br />
         <button className="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#4d93a2] to-[#9d84c7] text-white px-4 py-2 rounded-lg text-sm cursor-pointer">
          <Edit className="w-5"/>
          Generate Article
         </button>
      </form>
      ...
      <div className="w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]">
       ...
       <div className="flex-1 flex justify-center items-center">
           <div className="text-sm flex flex-col items-center gap-5 text-gray-500"> 
                    ...
                    <p className="text-md">Hit 'Generate Article' to see your words transform!</p>
           </div>
       </div>
      </div>
    </div>
